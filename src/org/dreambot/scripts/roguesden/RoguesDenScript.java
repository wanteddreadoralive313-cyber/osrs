...
public class RoguesDenScript extends AbstractScript {
    ...
    private final MazeStep[] MAZE_STEPS = {
        new MazeStep(new Tile(3047, 4973, 1), "Door", "Open", new Tile(3047, 4972, 1), -1),
        new MazeStep(new Tile(3048, 4970, 1), "Climb", "Climb", new Tile(3049, 4970, 1), -1),
        new MazeStep(new Tile(3050, 4970, 1), "Squeeze-through", "Squeeze", new Tile(3051, 4970, 1), -1),
        // --- Begin token path ---
        new MazeStep(new Tile(3052, 4970, 1), "Trap", "Jump-over", new Tile(3053, 4970, 1), -1),
        new MazeStep(new Tile(3054, 4970, 1), "Token", "Take", new Tile(3054, 4970, 1), -1),
        // --- Begin exit path ---
        new MazeStep(new Tile(3055, 4970, 1), "Exit door", "Open", new Tile(3056, 4970, 1), -1)
    };
    ...
    private void handleObstacle(MazeStep stepData) {
        GameObject obj = GameObjects.closest(o -> o != null && stepData.name.equals(o.getName()) && o.hasAction(stepData.action));
        if (obj != null && obj.interact(stepData.action)) {
            boolean success = Sleep.sleepUntil(() -> {
                boolean pos = stepData.successTile != null && getLocalPlayer().distance(stepData.successTile) <= 1;
                boolean anim = stepData.animationId != -1 && getLocalPlayer().getAnimation() == stepData.animationId;
                return pos || anim;
            }, 4000);
    ...
    private void prepareSupplies() {
        ...
        try {
            ...
            if (config.useStamina && !Inventory.contains(i -> i.getName().contains("Stamina potion"))) {
                boolean withdrew = getBank().withdrawAll(i -> i.getName().contains("Stamina potion"));
                if (withdrew) {
